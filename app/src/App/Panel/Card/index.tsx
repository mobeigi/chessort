...
export const Card = ({ moveDetail }: CardProps) => {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: moveDetail.uciMove });
    const transformStyle = CSS.Transform.toString(transform);
    const advantageColor = moveDetail.revealed ? 
      evaluateAdvantage(moveDetail.evalResults!!.engineEval) :
      Color.Neutral;

    const engineEvalValue = moveDetail.revealed ? 
      formatEvaluation(moveDetail.evalResults!!.engineEval) : 
      "?";
